...
int count(int gap, vector<int> &s)
{
	int ans = 0;
...
bool solve(){
	...
	vector<int> s(m);
	input(s,m);
	s.insert(s.begin(),1-gap);
	s.push_back(n+1);
	int Cookies = 0, lastCookie = -1;

	vector<int> r;
	int a, b, c, d, minCookies = 2e9;
	// calculting cookies after removal of i-th seller
	for(int i=1; i<=m; i++)
	{
		a = s[i] - s[i-1] -1;
		b = s[i+1] - s[i] - 1;
	...
	cout<<minCookies + count(gap, s) - 1<<" "<<r.size()<<nl;
	return 0;
}
